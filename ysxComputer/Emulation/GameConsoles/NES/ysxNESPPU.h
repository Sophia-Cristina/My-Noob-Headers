...
public:
    /*union regtype // PPU register file
    {
        ...
        RegBit<6, 1, uint32_t> SlaveFlag;  RegBit<11, 2, uint32_t> ShowBGSP;  RegBit<24, 2, uint32_t> OAMdata;
    ...
    union scrolltype
    {
        RegBit<3, 16, uint32_t> raw;       // raw VRAM address (16-bit)
        RegBit<0, 8, uint32_t> xscroll;   // low 8 bits of first write to 2005
        RegBit<0, 3, uint32_t> xfine;     // low 3 bits of first write to 2005
        RegBit<3, 5, uint32_t> xcoarse;   // high 5 bits of first write to 2005
        RegBit<8, 5, uint32_t> ycoarse;   // high 5 bits of second write to 2005
        RegBit<13, 2, uint32_t> basenta;   // nametable index (copied from 2000)
        RegBit<13, 1, uint32_t> basenta_h; // horizontal nametable index
        RegBit<14, 1, uint32_t> basenta_v; // vertical   nametable index
        RegBit<15, 3, uint32_t> yfine;     // low 3 bits of second write to 2005
        RegBit<11, 8, uint32_t> vaddrhi;   // first write to 2006 (with high 2 bits set to zero)
        RegBit<3, 8, uint32_t> vaddrlo;   // second write to 2006
    } scroll, vaddr;*/
    ...
    uint8_t Access(uint16_t index, uint8_t v, bool write)
    {
        auto RefreshOpenBus = [&](uint8_t v) { return open_bus_decay_timer = 77777, open_bus = v; };
        uint8_t res = open_bus;
