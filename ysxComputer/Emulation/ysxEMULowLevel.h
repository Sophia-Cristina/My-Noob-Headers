...
NPN: iC--v->oE | PNP: oC--v-<iE
	 iB    |          oB
Same as an 'AND'*/
//T_ TrnsBip(T_ In, T_ B) { return(In & B); } // Going to do it scalar

/*####### DE / MULTIPLEXER:
Data inputs are 2^Pow, and the switch inputs are Pow.*/
template <const size_t Pow, class T_>
class ysxEMU_Multiplexer
{
private:
    //uint8_t DataInputs[1 << Pow];
	//T_ SwitchInputs[Pow];
	T_* Data;
	T_* Switch;
...
public:
    ysxEMU_Multiplexer()
    {
        //for (size_t i = 0; i < (1 << Pow); ++i) { DataInputs[i] = 0; }
        // for (size_t i = 0; i < Pow; ++i) { SwitchInputs[i] = 0; }
    }

    // Function to set the value of a data input
    //void SetDataInput(size_t Index, uint8_t Val) { if (Index < (1 << Pow)) { Data[Index] = Val; } }

    // Function to set the value of a switch input
    // void SetSwitchInput(size_t Index, uint8_t Val) { if (Index < Pow) { Switch[Index] = Val; } }

    // Function to perform the multiplexing operation
    uint8_t Mux()
    {
        // Calculate the index based on switch inputs
        size_t Index = 0;
        for (size_t i = 0; i < Pow; ++i) { Index |= (Switch[i] << i); }

        // Return the selected data input
        if (Index < (1 << Pow)) { return(Data[Index]); }
        else { return(0); }
    }
};

class ysxEMU_Multiplexer4Bit
{
private:
public:
	uint8_t* Data;
