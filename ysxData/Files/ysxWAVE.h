...
public:
	...
	ysxWAV_RIFF RIFF; ysxWAV_FMT FMT; ysxWAV_DATA DATA; // Header chunks

	// #################################################
	// #################################################
	// #################################################

	ysxWAV_File(std::string OpenFile)
	{
	...
	ysxWAV_File(std::string FileName, uint32_t SampleRate, uint16_t Channels = 1, uint8_t BitsPerSample = 8)
	{
		if (wavFile.is_open()) { wavFile.close(); }
	...
	void ReadHeader()
	{
		wavFile.read((char*)&RIFF, sizeof(ysxWAV_RIFF));
		wavFile.read((char*)&FMT, sizeof(ysxWAV_FMT));
		wavFile.read((char*)&DATA, sizeof(ysxWAV_DATA));
		wavFile.seekg(0, std::ios::beg);
	}
	...
	void WriteHeader() // 12 24 8
	{
		...
		wavFile.write((char*)&RIFF, sizeof(ysxWAV_RIFF));
		wavFile.write((char*)&FMT, sizeof(ysxWAV_FMT));
		wavFile.write((char*)&DATA, sizeof(ysxWAV_DATA));
		wavFile.seekp(0, std::ios::beg);
	}

	// WRITE NEW HEADER:
	void WriteNewHeader(const ysxWAV_RIFF& SetRIFF, const ysxWAV_FMT& SetFMT, const ysxWAV_DATA& SetDATA) // 12 24 8
	{ RIFF = SetRIFF; FMT = SetFMT; DATA = SetDATA; WriteHeader(); }
	...
	void CoutHeaderInfo()
	{
		...
		std::cout << "sizeof(WAVEFile): " << sizeof(ysxWAV_File) << '\n';
	...
	std::string StringHeaderInfo()
	{
		...
		s += "sizeof(WAVEFile): " + std::to_string(sizeof(ysxWAV_File)) + '\n';
	...
	{
		wavFile.seekg(0, std::ios::end);
		uint64_t End = wavFile.tellg();
		wavFile.seekg(FileSize - DATA.Size * (FMT.BitsPerSample / 8)); // First sample
		uint64_t Size = End - (FileSize - DATA.Size * (FMT.BitsPerSample / 8)); // Size by input type
	...
	// GET DATA READY TO BE WORKED WITH AS A DOUBLE VECTOR, IGNORE LeftRight IF THERE IS ONLY ONE CHANNEL.
	...
	std::vector<double> GetDataReadyToUse(uint32_t Index, uint32_t Size, bool LeftRight = false)
	{
		...
		if (!(FMT.BitsPerSample % 8) && B == FMT.BitsPerSample / 8)
		{
			Index *= B; Size *= B;
			for (uint32_t n = Beg; n < End; )
			{				
				wavFile.read((char*)&Val, B);
		...
		else { std::cerr << "Bits per sample is either not equal to '2^n' or not the same size of 'T_'!"; }
		return(GETDATA);
	}
	...
	template <class T_> void SaveData(std::vector<double> In, bool LeftRight)
	{
		...
		for (size_t n = 0; n < Buf.size(); ++n)
		{
			...
			//{
				if (LeftRight) { wavFile.seekp((uint64_t)wavFile.tellp() + Bs); }
				Buf[n] = round((In[n] + 1) * 0.5 * (pow(256, sizeof(T_)) - 1));
				wavFile.write((char*)&Buf[n], sizeof(T_));
				if (!LeftRight) { wavFile.seekp((uint64_t)wavFile.tellp() + Bs); }
			//}
	...
	// APPEND DATA:
	template <class T_> void AppendData(std::vector<double> In)
	{
		size_t Is = In.size();
		std::vector<T_> Buf(Is);
	...
	template <class T_> void AppendData(std::vector<double> In, bool LeftRight)
	{
		size_t Is = In.size();
		std::vector<T_> Buf(Is);
