...
//#include "ysxCImg.h" // Some output needs it, SO BE SURE TO INCLUDE IT IF NOT ALREADY
...
public:
    ...
    ysxELEC_PowerDC(ysxELEC_CompConfig& InConfig)
    {
        ...
        Config = &InConfig;
        for (size_t n = 0; n < Sig.size(); ++n) { Sig[n] = Config->Volts - Config->NoiseGain + Config->NoiseGain * (((rand() % 20001) - 10000) / 10000.0); }
    }
...
public:
    ...
    void ProcessSignal() override
    {
        ...
        for (uint32_t n = 0; n < Output->Signals[0].size(); ++n) { Output->Signals[0][n] = 1; } // W.I.P
    }
};
