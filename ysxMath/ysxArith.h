...
// FACTORIAL:
size_t ysxARITH_Factorial(size_t a) { size_t Fact = 1; if (a > 0) { for (size_t n = 1; n <= a; ++n) { Fact *= n; } return (Fact); } else { return (1); } }
...
int ysxARITH_DivFunc(int n, int Power) { std::vector<int> Fac = ysxVEC_PowVec(ysxARITH_Factors(n), Power); return(ysxVEC_SumVec(Fac)); }
...
// ARITHMETIC SEQUENCE:
std::vector<double> ysxARITH_ArithSeq(double a, double d, size_t n)
{ std::vector<double> S{ a }; for (size_t k = 1; k < n; ++k) { S.push_back(S[0] + (d * k)); } return (S); }
constexpr double ysxARITH_ArithSeqnth(double a, double d, int n) { return(a + d * (n - 1.0)); }
constexpr double ysxARITH_ArithSeqSum(double a, double d, int n) { return(n * 0.5 * (2.0 * a + d * (n - 1.0))); }
// Get the difference from an arithmetic sequence which results as "1" at "n":
constexpr double ysxARITH_ArithSeqDiffAt1(double a, int n) { return((1 - a) / (n - 1.0)); }
constexpr double ysxARITH_ArithSeqDiffAtb(double a, double b, int n) { return((b - a) / (n - 1.0)); }

// GEOMETRIC SEQUENCE:
std::vector<double> ysxARITH_GeoSeq(double a, double r, int n)
{ std::vector<double> S{ a }; for (int k = 1; k < n; ++k) { S.push_back(S[0] * pow(r, k)); } return (S); }
double ysxARITH_GeoSeqnth(double a, double r, int n) { return(a * pow(r, n - 1.0)); }
double ysxARITH_GeoSeqSum(double a, double r, int n) { return(a * ((1.0 - pow(r, n)) / (1.0 - r))); }
double ysxARITH_GeoSeqProduct(double a, double r, int n) { return(pow(sqrt(a * a * pow(r, n)), n + 1.0)); } // https://en.wikipedia.org/wiki/Geometric_progression#Product
double ysxARITH_GeoSeqRatioAt1(double a, int n) { return(pow((1 / a), 1 / (n - 1.0))); }
double ysxARITH_GeoSeqRatioAtb(double a, double b, int n) { return(pow((b / a), 1 / (n - 1.0))); }

// EASY SEQUENCE FORMULA ((a^(b + c))^d) / e):
double ysxARITH_EzSequenceForm(double a, double b, double c, double d, double e) { return(pow(pow(a, b + c), d) / e); }
