...
public:
	...
	void GetRatios()
	{
		...
		else if (BiggerSide == 'n') { Ratios[0] = 1; Ratios[1] = 1; Ratios[2] = 1; }
	};
	// ############################
	double a, b, c; // Sides
	...
	void CoutInfo()
	{
		...
		std::cout << "Tipo triangulo = " << LType << " & " << AType << " | Bigger: " << BiggerSide << std::endl;
	...
	ysxTRI_Triangle(double asize, double bsize, double Rad)
	{
		...
		// Ratios, and automatically get the bigger size:
		GetRatios();
		if (BiggerSide == 'n') { LnghtType = Equilateral; }
