...
// !!!!!!!	
// !!!!!!!	CHANGES:
// !!!!!!!	* 'MiniForm(...)' is no longer available! Functions that used it are now receiving function pointer as parameter;
// !!!!!!!	
// ################################################# NOTES #################################################
// ############################################################################################################################################
...
constexpr double ysxCALC_ArithSeqnth(double a, double d, int n) { return(a + d * (n - 1.0)); }
constexpr double ysxCALC_ArithSeqSum(double a, double d, int n) { return(n * 0.5 * (2.0 * a + d * (n - 1.0))); }
// Get the difference from an arithmetic sequence which results as "1" at "n":
constexpr double ysxCALC_ArithSeqDiffAt1(double a, int n) { return((1 - a) / (n - 1.0)); }
constexpr double ysxCALC_ArithSeqDiffAtb(double a, double b, int n) { return((b - a) / (n - 1.0)); }
