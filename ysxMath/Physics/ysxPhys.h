...
//
// CHANGES (KEEP ORDER):
// * This will be divided by areas of physic in the future;
// 
// ################################################# ANOTATIONS AND ALTERATIONS #################################################
// ############################################################################################################################################
...
// #####################################################################################################################################
// ####### DECLARATIONS:
constexpr double ysxPHYS_DensityAir(double, double, double);
...
constexpr double ysxPHYS_Joules(double NorWorPaorC, double morsorcubemorV) { return(NorWorPaorC * morsorcubemorV); } // (Newton or Watts or Pa or Coulomb) * (meter or seconds or meter^3 or Volts) | Energy
constexpr double ysxPHYS_Joules(double kg, double m, double s) { return((kg * m * m) / (s * s)); } // (kg * meter^2) / s^2 | Energy
constexpr double ysxPHYS_Watts(double J, double s) { return(J / s); } // Its simple Joules per sec | Power
constexpr double ysxPHYS_Watts(double N, double m, double s) { return((N * m) / s); } // (Newton * meter) / seconds | Power
constexpr double ysxPHYS_Newtons(double m) { return(m * GRAVITY); } // Newton = mass * Gravity_accel | Force
constexpr double ysxPHYS_Newtons(double m, double a) { return(m * a); } // Newton = mass * acceleration | Force
constexpr double ysxPHYS_Newtons(double kg, double meter, double s) { return((kg * meter) / (s * s)); } // Newton = (kg * meter) / s^2 | Force
double ysxPHYS_Tension(double kg, double rad) { return((kg * GRAVITY) / sin(rad)); } // (kg * 9.80665) / sin(rad)
constexpr double ysxPHYS_Momentum(double kg, double ms) { return(kg * ms); }
constexpr double ysxPHYS_Pressure(double Force, double Area) { return(Force / Area); }
constexpr double ysxPHYS_NormForce(double kg) { return(kg * GRAVITY); } // Normal force
...
*/
constexpr double ysxPHYS_Accel(double F, double m) { return(m == 0 ? 0 : F / m); } // Acceleration can be written as Force / mass
// BASE:
constexpr double ysxPHYS_DistTrav(double Vel0, double t, double Accel) { return((Vel0 * t) + (0.5 * Accel * t * t)); } // Distance traveled
...
constexpr double ysxPHYS_SpeedWindHghtGrad(double h0, double h, double SurfRough) { return(h == 0 ? 0 : SurfRough * (ysxPHYS_SpeedWindHeight(h0, h, SurfRough) / h)); }

// PARTICLE:

// useless:
//constexpr double ysxPHYS_ParticleVel(double d1, double d2, double t1, double t2) { return((d2 - d1) / (t2 - t1)); } // d = displacement; t = time;

// KINETIC:
constexpr double ysxPHYS_KineticEnrg(double kg, double ms) { return(0.5 * kg * ms * ms); } // Speed in m/s | return = Joules
constexpr double ysxPHYS_KineticEnMom(double p, double kg) { return(kg == 0 ? 0 : (p * p) / (2 * kg)); } // The kinetic energy of an object is related to its momentum
constexpr double ysxPHYS_Density(double kg, double Vol) { return(kg / Vol); } // Densidade, kg/m^3
// Pa = Pressure; Gas Constant in J/(kg·K); T = Temperature in Kelvins; care with div by 0
constexpr double ysxPHYS_DensityAir(double Pa, double GasConst, double T) { return((Pa / (GasConst * T))); }
constexpr double ysxPHYS_Stiffness(double F, double d) { return(d == 0 ? 0 : F / d); } // Force / Displacement (menor distancia entre x1 e x2)| https://en.wikipedia.org/wiki/Displacement_(vector)

// ####### ELASTICITY AND ETC:
// P = pressure | p = density | change p (elasticity) to v if needed
// I don't remember i i accidentaly changed this formula or not...
constexpr double ysxPHYS_BulkMod(double P1, double P2, double p1, double p2) { return((P2 - P1) / (- (p2 - p1) / p1)); }
...
// #####################################################################################################################################
// SOUND SPEED:
constexpr double ysxPHYS_SpeedSound(double BulkMod, double p) { return(p == 0 ? 0 : sqrt(BulkMod / p)); } // BulkMods; p = density
constexpr double ysxPHYS_SpeedSoundLiq(double BulkMod, double p) { return(p == 0 ? 0 : sqrt(BulkMod / p)); } // BulkMods; p = density | Same as above, but Bulk Mod. of the fluid
...
constexpr double ysxPHYS_SpeedSndDryAir(double Celsius) { return(331.3 + 0.606 * Celsius); } // Return in m/s
// https://en.wikipedia.org/wiki/Speed_of_sound#Speed_of_sound_in_solids // pw = P-Waves, primary waves; sw = S-Waves, secondary waves;
constexpr double ysxPHYS_SpeedSndSolidpwBulk(double Bulk, double Shear, double Density)
{ return(Density == 0 ? 0 : sqrt((Bulk + (1.333333333 * Shear)) / Density)); } // Bulk formula
constexpr double ysxPHYS_SpeedSndSolidpwYoung(double Young, double Poisson, double Density)
{ return(Density == 0 ? 0 : (Young * (1 - Poisson)) / (Density * (1 + Poisson) * (1 - (2 * Poisson)))); } // Young formula
constexpr double ysxPHYS_SpeedSndSolidsw(double Shear, double Density) { return(Density == 0 ? 0 : sqrt(Shear / Density)); }
constexpr double ysxPHYS_SpeedSndSolid1d(double Young, double Density) { return(Density == 0 ? 0 : sqrt(Young / Density)); }
...
constexpr double ysxPHYS_SpeedSndPlasma(double mi, double Te, double Z, double k, double y)
{ return(mi == 0 ? 0 : pow((y * Z * Te) / mi, 0.5)); } // Return in m/s
double ysxPHYS_SpeedSndPlasma0(double Mu, double Te, double Z, double k, double y)
{ return((Mu == 0 ? 0 : 9.79 * pow(10, 3)) * pow((y * Z * Te) / Mu, 0.5)); } // Return in m/s
constexpr double ysxPHYS_SingleShotTime(double CapDist, double Time) { return(Time == 0 ? 0 : CapDist / Time); } // Time amount sound takes to be captured in CapDist (like a microphone) shows sound velocity

// #####################################################################################################################################
// SOUND INTENSITY:
constexpr double ysxPHYS_SoundIntpv(double p, double v) { return(p * v); } // p = Sound pressure; v = Particle velocity;
constexpr double ysxPHYS_SoundIntPA(double Watts, double Area) { return(Area == 0 ? 0 : Watts / Area); } // Watts / m^2
 // Inverse-square law, for Spherical sound intensity, 4 * PI * r^2 = surface area of a sphere:
constexpr double ysxPHYS_SoundIntSph(double Watts, double r) { return(r == 0 ? 0 : Watts / (4 * PI * r * r)); }
// Watt / 4PIr^2 = (Joules / s) / 4PIr^2; Care with div by 0:
constexpr double ysxPHYS_SoundIntSph(double Newtons, double meters, double seconds, double r)
{ return(((Newtons * meters) / seconds) / (4 * PI * r * r)); }

// WAVE LENGTH (Lambda):
constexpr double ysxPHYS_WaveLgh(double v, double f) { return(f == 0 ? 0 : v / f); } // v = Phase velocity; f = Frequency;

// SOUND PRESSURE:
constexpr double ysxPHYS_SoundPress(double I, double v) { return(v == 0 ? 0 : I / v); }
constexpr double ysxPHYS_SoundPress(double Watts, double Area, double v) { return(v == 0 ? 0 : (Watts / Area) / v); }
...
double ysxPHYS_AcoWavEq(double d2p, double dx2, double dt2)
{ return((d2p / dx2) - (1.0 / pow(ysxPHYS_SpeedSndAir(101325, 101325, 288.15, 287.058), 2)) * (d2p / dt2)); }

// WAVE EQUATION (make it sinuoid):
...
constexpr double ysxPHYS_SimpleHarmOsc(double Tht0, double T, double t, double phi) { return(T == 0 ? 0 : Tht0 * cos(phi + ((2 * PI) / T) * t)); }
