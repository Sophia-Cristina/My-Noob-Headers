	...
	void Polar(double Amp, double Tht1, double Tht2, int div, bool Deriv, bool Comraio, bool Tri, bool Borda, bool LRGB)
{
	...
	// PLOTAR POINCARÉ:
	void PoincareMap(double Amp, double t1, double t2, double Periodo, bool PrintPath, bool LRGB)
	{
		int TR = R, TG = G, TB = B;
		double y, x;
		double t, T = 0, xt, xtT;
		...
		for (T = 0; T <= Tau; T += Periodo /*1 / Periodo*/)
		{
			cout << "T: " << T << endl;
			for (t = t1; t <= t2; t += Periodo)
			{
	...
	void TMap(double Amp, double t1, double t2, double Periodo, double PAmp, int Mod, bool PrintPath, bool LRGB)
	{
		...
		for (t = t1; t <= t2; t += 0.0001)
		{
			...
			xt = Amp * (cos(ModForm(t + sin(t), 1) * Tau) + sin(IntgrlRAM(0, t, 1, 100, 1))) * 0.5;
			...
			if (Mod == 3)
			{
				y = round(IntgrlRAM(0, (1.0 / 3) * (t + xt + T), 1, 100, 1 ) * (xt + xtT) * 0.5 * sin(t));
				x = round(IntgrlRAM(0, (1.0 / 3) * (t + xt + T), 1, 100, 1) * (xt + xtT) * 0.5 * cos(t));
			}
			if (Mod == 4)
			{
				y = round((xt + xtT) * 0.5 * sin(t));
