...
// !!!	* !!! HUGE: Changing / changed all "uint8_t* C" to "C = nllptr", and if "!C", then use "LinearRGBuc", and delete redundant functions !!!
// ################################################# ANOTAÇÕES E ATENÇÕES #################################################
// ############################################################################################################################################
...
void Circlexy(CImg<uint8_t>& Img, double r, uint16_t x, uint16_t y, uint8_t* C = nullptr)
{
	uint16_t yc, xc;
	...
	for (double rad = 0; rad <= TAU; rad += dt) // MUDAR SE NESCESSARIO
	{
		...
		if (InImg(Img, yc, xc))
		{
			if (!C)	{ LinearRGBuc(rad / TAU, 1, 1, Cc); Img.draw_point(xc, yc, Cc);	}
...
void Circle(CImg<uint8_t>& Img, uint8_t* C = nullptr)
{
...
void Polar(CImg<uint8_t>& I, double r, double x, double y, double t1, double t2, double Omega, uint8_t* C = nullptr)
{
	...
	for (double t = t1; t <= t2; t += dt)
	{
		...
		if (!C) { LinearRGBuc((t - t1) / (t2 - t1), 1, 1, c); I.draw_point(xc, yc, c); }
		else { I.draw_point(xc, yc, C); }
	}
}

// HYPOCYCLOID (FIX PERIMETER OR MAYBE THE PROBLEM IS ON THE COORDS):
void HypoCicl(CImg<uint8_t>& I, double R, double r, double x, double y, double t1, double t2, uint8_t* C = nullptr)
{
...
void EpiCicl(CImg<uint8_t>& I, double R, double r, double x, double y, double t1, double t2, uint8_t* C = nullptr)
{
	if (t1 < 0) { t1 = TAU + t1; } if (t2 < t1) { double T = t1; t1 = t2; t2 = T; }
...
// TURN:
void Turn(CImg<uint8_t>& I, double r, uint16_t x, int16_t y, double Ini, double Turn, uint8_t* C = nullptr)
{
	uint16_t yc, xc;
	...
	for (rad = Ini; rad <= Turn; rad += dt)
	{
		...
		if (InImg(I, yc, xc))
		{
			if (!C) { LinearRGBuc((rad - Ini) / (Turn - Ini), 1, 1, c); I.draw_point(xc, yc, c); }
			else { I.draw_point(xc, yc, C); }
		}
	}
}
...
// PLOT RADIUS:
void Radius(CImg<uint8_t>& Img, double r, uint16_t x, int16_t y, double rad, bool Triangle, bool Border, uint8_t* C = nullptr)
{
	uint16_t xc, yc, rn;
	...
	if (Border) { if (!C) { Circlexy(Img, r, x, y, c); } else { Circlexy(Img, r, x, y, C); } }
}
...
void RadiusPolygon(CImg<uint8_t>& Img, double r, uint16_t x, int16_t y, double OffSet, int Divisions, bool Triangle, bool Border, uint8_t* C = nullptr)
{
	double Div = TAU / Divisions;
	for (double n = OffSet; n < TAU + OffSet; n += Div)
	{
		if (!C) { Radius(Img, r, x, y, n, Triangle, Border); }
		else { Radius(Img, r, x, y, n, Triangle, Border, C); }		
	}
}
...
// PLOT LINE:
void Line(CImg<uint8_t>& Img, double r, double rad, uint16_t x, uint16_t y, bool Triangle, uint8_t* C = nullptr)
{
	...
	for (rn = 0; rn <= r; rn = ++rn)
	{
		...
		if (InImg(Img, yc, xc))
		{
			if (!C) { LinearRGBuc(rn / r, 1, 1, c);	Img.draw_point(xc, yc, c); }
			else { Img.draw_point(xc, yc, C); }
			if (Triangle)
			{
				if (!C) { IC[0] = 255 - c[0]; IC[1] = 255 - c[1]; IC[2] = 255 - c[2]; }
...
// BRESENHAM LINE:
void Linexy(CImg<uint8_t>& Img, int x1, int y1, int x2, int y2, uint8_t* C = nullptr)
{
	...
	if (dy1 <= dx1)
	{
		...
		for (i = 0; x < xe; i++)
		{
			...
			if (!C) { LinearRGBuc(x / (xe - 1.0), 1, 1, c); Img.draw_point(x, y, c); }
	...
	else
	{
		...
		for (i = 0; y < ye; i++)
		{
			...
			if (!C) { LinearRGBuc(y / (ye - 1.0), 1, 1, c); Img.draw_point(x, y, c); }
			else { Img.draw_point(x, y, C); }
		}
	}
}
...
// PLOT POLYGON:
void Polygon(CImg<uint8_t>& Img, int Sides, double r, uint16_t x, uint16_t y, bool Vertex, uint8_t* C = nullptr)
{
	...
	for (double n = 0.0; n <= TAU; n = n + Div)
	{
		...
		if (Vertex)
		{
			if (!C) { LinearRGBuc(n / TAU, 1, 1, c); AddVert(Img, xc, yc, 3, c); }
	...
	for (uint16_t ns = 1; ns < Sides; ++ns)
	{
		...
		{
			if (!C) { LinearRGBuc(ns - 1 / Sides, 1, 1, c); Linexy(Img, Coord[ns - 1].x, Coord[ns - 1].y, Coord[ns].x, Coord[ns].y, c); }
			else { Linexy(Img, Coord[ns - 1].x, Coord[ns - 1].y, Coord[ns].x, Coord[ns].y, C); }
			if (ns == Sides - 1) { Linexy(Img, Coord[ns].x, Coord[ns].y, Coord[0].x, Coord[0].y, c); }
		}
	}
...
CImg<uint8_t> PrintVectorLine(std::vector<double> V, int SegSize, uint8_t BG, uint8_t* C)
{
	...
	{ Linexy(Print, (n - 1)  * SegSize, MaxMin - ((V[n - 1] + Absmn) * SegSize), n * SegSize, MaxMin - ((V[n] + Absmn) * SegSize), C); }
...
// REDO IT!!!
CImg<uint8_t> PrintVectorPolygn(std::vector<double> V, uint8_t BG, bool Vertex, uint8_t* C = nullptr)
{
	...
	{
		Linexy(Ret, xy[ns - 1].x, xy[ns - 1].y, xy[ns].x, xy[ns].y, C); // LINES
		if (ns == Sides - 1) { Linexy(Ret, xy[0].x, xy[0].y, xy[ns].x, xy[ns].y, C); } // LAST LINE
		if (Vertex) // VERTEXES
		{
			if (!C) { double Val = ns / Sides; LinearRGBuc(Val, 1, 1, c); AddVert(Ret, xy[ns].x, xy[ns].y, 3, c); }
...
template <class T_>
CImg<uint8_t> PrintVectorPointNorm(std::vector<T_>& V, uint32_t sy, double yrto = 1)
{
	...
	for (size_t n = 0; n < Size; ++n)
	{
		...
		{
			LinearRGBuc(V[n] / (double)(pow(256, sizeof(T_)) - 1), 1, 1, C);
...
template <class T_>
CImg<uint8_t> PrintVectorPointNorm(std::vector<T_>& V)
{
	...
	{
		ndiv = (float)n / (Size - 1);
		if (std::is_floating_point<T_>) { LinearRGBuc(V[n], 1, 1, C); }
		else { LinearRGBuc(V[n] / Max, 1, 1, C); }
...
template <class T_>
CImg<uint8_t> PrintVectorPointNorm(std::vector<T_>& V, uint32_t sy, uint8_t* C, double yrto = 1)
{
	uint32_t Size = V.size(); CImg<uint8_t> R(Size, sy, 1, 3, 0);
...
void PrintVectorLineOnImg(CImg<uint8_t>& Img, std::vector<double>& V, double yrto = 1, uint8_t* C = nullptr)
{
	...
	for (size_t n = 1; n < Size; ++n)
	{
		...
		if (!C) { LinearRGBuc(V[n], 1, 1, c); Linexy(Img, x0, y0, x1, y1, c); }
		else { LinearRGBuc(V[n], 1, 1, c); Linexy(Img, x0, y0, x1, y1, C); }
	}
}

void PrintVectorPointOnImg(CImg<uint8_t>& Img, std::vector<double>& V, double yrto = 1, uint8_t* C = nullptr)
{
	...
	for (size_t n = 0; n < Size; ++n)
	{
		...
		if (!C) { LinearRGBuc(V[n], 1, 1, c); Img.draw_point(x, y, c); }
		else { Img.draw_point(x, y, C); }
	}
}
void PrintVectorPtOnImgPolar(CImg<uint8_t>& Img, std::vector<double> V, double Ratio, uint8_t* C = nullptr)
{
	uint32_t Size = V.size(), sy = Img.height(), sx = Img.width();
	double Max, Min; MaxMinVecAbs(V, Max, Min);
	double dx = sx / (double)Size;
	...
	for (size_t n = 0; n < Size; ++n)
	{
		...
		if (y > sy) { y = sy; } if (y < 0) { y = 0; }
		if (x > sx) { x = sx; } if (x < 0) { x = 0; }
		if (!C) { LinearRGBuc((float)n / Size, 1, 1, c); Img.draw_point(x, y, c); }
		else { Img.draw_point(x, y, c); }
	}
}

// PLOT LINE BETWEEN POINTS IN A COORDINATE VECTOR:
CImg<uint8_t> PrintPoints(std::vector<Point<int>> Coord, int SegSize, uint8_t BG, bool Abs, uint8_t* C = nullptr)
{
	Point<int> max, min;
...
CImg<uint8_t> PrintLinePoints(std::vector<LinePoint<int>> Coord, int SegSize, uint8_t BG, bool Abs, uint8_t* C = nullptr) // Testar para ver se esta tudo certo
{
	...
	for (size_t n = 0; n < Coord.size(); ++n)
	{
		if (!C)
		{
			LinearRGBuc((float)n / Coord.size(), 1, 1, c);
			Linexy(Print, Coord[n].P0.x * SegSize, Coord[n].P0.y * SegSize, Coord[n].P1.x * SegSize, Coord[n].P1.y * SegSize, c);
		}
		else { Linexy(Print, Coord[n].P0.x * SegSize, Coord[n].P0.y * SegSize, Coord[n].P1.x * SegSize, Coord[n].P1.y * SegSize, C); }
	}
	return(Print);
}
...
template <class T_> CImg<uint8_t> PrintVectorImg(std::vector<Point3D<T_>>& V, uint16_t x, int16_t y, double Rsize = 1.0)
{
...
template <class T_> CImg<uint8_t> PrintVectorImg(std::vector<Point3D<T_>>& V, double Rsize = 1.0)
{
...
template <class T_> CImg<uint8_t> PrintVectorInfoLRGB(std::vector<T_>& V, double Rsize = 1.0)
{
	uint32_t Size = V.size(); double SqrtVec = sqrt(Size); uint16_t x = round(SqrtVec), y;
	...
	for (size_t n = 0; n < Size; ++n)
	{
		if (std::is_floating_point<T_>) { LinearRGBuc(V[n], 1.0, 1.0, C); }
		else { LinearRGBuc(V[n] / (float)(pow(256, sizeof(T_)) - 1), 1.0, 1.0, C); }
		R.draw_point(n % x, floor((float)n / x), C);
	}
	if (Rsize != 1.0) { Resize(R, round(x * Rsize), round(y * Rsize), 6); }
	return(R);
}
template <class T_> CImg<uint8_t> PrintVectorInfoLRGB(std::vector<T_>& V, int x, double Rsize = 1.0)
{
	uint32_t Size = V.size();
...
CImg<uint8_t> PrintFileAsBMP(std::string Path, double Multiplier, double Rsize = 1.0)
{
	...
	if (Size > 0)
	{
		for (size_t n = 0; n < Size; ++n)
		{
			LinearRGBuc(((uint8_t)V[n] / 255.0) * Multiplier, 1.0, 1.0, c);
			R.draw_point(n % x, floor((float)n / x), c);
		}
		if (Rsize != 1.0) { Resize(R, round(x * Rsize), round(y * Rsize), 6); }
	}
...
public:
	...
	// ##################### CONSTRUTORES #####################
	TriPrint(Triangle TriInput, double ScaleRto, int BorderImg, uint8_t* inColor = nullptr)
	{
	...
	void NewTri(Triangle TriInput) // RE-DO THIS!!!
	{
	...
	{
		Linexy(TriOut, x[0], y[0], x[1], y[1], Color); // a -> "_"
		if (!ChangeSidesColor) { Linexy(TriOut, x[1], y[1], x[2], y[2], Color); }
		else
		{
			uint8_t nc[] = { (Color[0] + 85) % 256, (Color[1] + 85) % 256, (Color[2] + 85) % 256 };
			Linexy(TriOut, x[1], y[1], x[2], y[2], nc);
		} // c -> "\"
		if (!ChangeSidesColor) { Linexy(TriOut, x[2], y[2], x[0], y[0], Color); }
		else
		{
			uint8_t nc[] = { (Color[0] + 85) % 256, (Color[1] + 85) % 256, (Color[2] + 85) % 256 };
			Linexy(TriOut, x[2], y[2], x[0], y[0], nc);
		}	// b -> "/"
		if (Text)
		{
		...
		if (AddHeight)
		{
			Linexy(TriOut, (SclCrd[2].x) + Border, Offy, (SclCrd[2].x) + Border, Offy - round(Scale * h), ClrhLine);
			if (Text)
