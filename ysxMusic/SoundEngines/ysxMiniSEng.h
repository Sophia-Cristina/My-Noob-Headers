...
The sampling system keeps overwritting a space in 'BUF' while 'n < Samples'.
...
{
	/* MAIN BUFFER:
	MSA1 can't play stereo. But if you don't care about hardware accuracy,
	then you can declare another array on your 'main.cpp', then copy and
	modify 'MSA1.DAC()' so that it also register to the new array.*/
...
public:
	...
	void RESET()
	{
		...
		RAM->n = 0; RAM->x = 0; RAM->g = 191; RAM->y = 0; RAM->b = 0;
		...
		for (uint8_t I = 0; I < INSTR; ++I)
		{
			if (SigPins[I])
			{
				...
				for (uint8_t C = 0; C < SigPins[I]->C.size(); ++C)
				{
					if (SigPins[I]->C[C])
					{
						SigPins[I]->Freq = ROM->MtoF[SigPins[I]->MIDI[C].x]; // Sets frequency, can be ignored at hardware level
						SigPins[I]->V = C; // Send number of the voice in use
						RAM->y += SigPins[I]->IO(RAM->x) / (float)RAM->Gg; // Gets audio sample and divide the gain
