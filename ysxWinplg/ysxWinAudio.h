...
WAVEFORMATEX SetWaveFmt(std::string FileName)
{
	...
	Wave.wFormatTag = WAVE_FORMAT_PCM;
	Wave.nChannels = File.FMTChnk.NumChannels;
	Wave.nSamplesPerSec = File.FMTChnk.SampleRate;
	Wave.nAvgBytesPerSec = File.FMTChnk.ByteRate;
	Wave.nBlockAlign = File.FMTChnk.BlockAlign;
	Wave.wBitsPerSample = File.FMTChnk.BitsPerSample;
	Wave.cbSize = 0;
	return (Wave);
}
WAVEFORMATEX SetWaveFmt(WAVEFile File)
{
	WAVEFORMATEX Wave;
	Wave.wFormatTag = WAVE_FORMAT_PCM;
	Wave.nChannels = File.FMTChnk.NumChannels;
