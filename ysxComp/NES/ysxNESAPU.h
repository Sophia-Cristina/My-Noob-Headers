...
[3] https://www.nesdev.org/wiki/CPU_pinout
[4] https://www.nesdev.org/wiki/APU // Audio Processing Unit
*/
// #################################################
...
{
public:
        static const uint8_t LengthCounters[32] = { 10,254,20, 2,40, 4,80, 6,160, 8,60,10,14,12,26,14,12, 16,24,18,48,20,96,22,192,24,72,26,16,28,32,30 };
        static const uint16_t NoisePeriods[16] = { 2,4,8,16,32,48,64,80,101,127,190,254,381,508,1017,2034 };
        static const uint16_t DMCperiods[16] = { 428,380,340,320,286,254,226,214,190,160,142,128,106,84,72,54 };
        ...
        struct channel
        {
            ...
            union // Per-channel register file
            {
                ...
                RegBit<0, 8, uint32_t> reg0;                 RegBit< 8, 8, uint32_t> reg1;          RegBit<16, 8, uint32_t> reg2;
                RegBit<6, 2, uint32_t> DutyCycle;            RegBit< 8, 3, uint32_t> SweepShift;    RegBit<16, 4, uint32_t> NoiseFreq;
                RegBit<4, 1, uint32_t> EnvDecayDisable;      RegBit<11, 1, uint32_t> SweepDecrease; RegBit<23, 1, uint32_t> NoiseType;
                RegBit<0, 4, uint32_t> EnvDecayRate;         RegBit<12, 3, uint32_t> SweepRate;     RegBit<16, 11, uint32_t> WaveLength;
                RegBit<5, 1, uint32_t> EnvDecayLoopEnable;   RegBit<15, 1, uint32_t> SweepEnable;   // 4003, 4007, 400B, 400F, 4010:
                RegBit<0, 4, uint32_t> FixedVolume;          RegBit< 8, 8, uint32_t> PCMlength;     RegBit<24, 8, uint32_t> reg3;
                RegBit<5, 1, uint32_t> LengthCounterDisable;                                 RegBit<27, 5, uint32_t> LengthCounterInit;
                RegBit<0, 7, uint32_t> LinearCounterInit;                                    RegBit<30, 1, uint32_t> LoopEnabled;
                RegBit<7, 1, uint32_t> LinearCounterDisable;                                 RegBit<31, 1, u32> IRQenable;
            } reg;
                    ...
                    if (ch.phase == 0) // Nothing in sample buffer?
                    {
                        ...
                        if (ch.length_counter > 0) // Load next 8 bits if available
                        {
                            ...
                            if (ch.reg.WaveLength > 20)
                                for (unsigned t = 0; t < 3; ++t) CPU::RB(uint16_t(ch.address) | 0x8000); // timing
                            ch.hold = CPU::RB(uint16_t(ch.address++) | 0x8000); // Fetch byte
                            ch.phase = 8;
                            --ch.length_counter;
                        }
