...
template <class T_> void ysxVEC_SubstituteVals(std::vector<T_>& Dest, std::vector<T_> In, uint32_t Index)
{
	uint32_t Size = (Index + In.size()) > Dest.size() ? Dest.size() - Index : In.size();
...
double ysxVEC_MaxVecAbs(std::vector<double> V) { double Max = V[0]; for (size_t n = 1; n < V.size(); ++n) { if (fabs(V[n]) > Max) Max = fabs(V[n]); } return(Max); }
...
double ysxVEC_MinVecAbs(std::vector<double> V) { double Min = V[0]; for (size_t n = 1; n < V.size(); ++n) { if (fabs(V[n]) < Min) Min = fabs(V[n]); } return(Min); }
...
void ysxVEC_MaxMinVecAbs(std::vector<int>& V, int& Max, int& Min)
{
	...
	if (max < 0) { min += abs(max); max = 0; } // max += fabs(max); }
	if (min < 0) { max += abs(min); min = 0; } // min += fabs(min); }
	Max = max; Min = min;
}
...
void ysxVEC_MaxMinVecAbs(std::vector<float>& V, float& Max, float& Min) // Soma com ABS, não ABS de apenas ABS.
{
