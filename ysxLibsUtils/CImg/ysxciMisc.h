...
// ############################################################################################################################################
// ############## MODULATIONS WITH COM HUE:
CImg<uint8_t> PaintHueRGBGapMod(CImg<uint8_t> I, double Phase, double Omega, double (*f)(double, double), uint8_t* RGB0, uint8_t* RGB1)
{
	std::vector<Pixel> VP = ysxCIMG_CLR_BitmapPixelMatrix(I);
	uint8_t C[3]; double Hue = 0;
	for (uint32_t n = 0; n < VP.size(); ++n)
	{
		if (ysxCIMG_CLR_InsideRGBGap(VP[n].RGB, RGB0, RGB1))
		{
			Hue = ysxCALC_ModForm(((double)n / VP.size() * TAU) + Phase, Omega, f);
			ysxCIMG_CLR_LinearRGBuc(Hue, 1, 1, C); I.draw_point(VP[n].x, VP[n].y, C);
		}
	}
	return(I);
}
...
CImg<uint8_t> ysxIMGMISC_SieveEratosthenes(uint32_t  n)
{
	if (n < 10) { n = 10; }
	CImg<uint8_t> Ret(320, ceil(n / 10.0) * 32, 1, 3, 0);
	CImg<uint8_t> Square;
	std::vector<Point3D<uint8_t>> RGBs(4);
	for (uint8_t m = 0; m < 4; ++m) { RGBs[m] = ysxCIMG_CLR_LinearRGB(m / 4.0, 1, 1); }
	uint8_t C[] = { 0, 0, 0 };
...
// RAY INFO:
CImg<uint8_t> ysxIMGMISC_RayInfo(double Degrees, uint32_t  ImgSize)
{
