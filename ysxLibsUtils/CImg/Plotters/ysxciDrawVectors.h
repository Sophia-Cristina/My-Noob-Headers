...
// ############## VETORES:
// THREE VECTORS TO RGB (soon to have template instead of being only 'double'):
...
CImg<uint8_t> ysxCIMG_DRAW_VectorLine(std::vector<double> V, int SegSize, uint8_t BG, uint8_t* C = nullptr)
{
	...
	{
		if (C) { ysxCIMG_DRAW_Linexy(Print, (n - 1) * SegSize, MaxMin - ((V[n - 1] + Absmn) * SegSize), n * SegSize, MaxMin - ((V[n] + Absmn) * SegSize), C); }
		else
		{
			uint8_t Clr[3] = { 0, 0, 0 }; ysxCOLOR_LinearRGB(V[n], 1, 1, Clr);
			ysxCIMG_DRAW_Linexy(Print, (n - 1) * SegSize, MaxMin - ((V[n - 1] + Absmn) * SegSize), n * SegSize, MaxMin - ((V[n] + Absmn) * SegSize), Clr);
		}
	}
...
CImg<uint8_t> ysxCIMG_DRAW_VectorPoint(std::vector<double> V, uint8_t BG, uint8_t* C = nullptr)
{
	uint32_t Size = V.size();
	double Max, Min; ysxVEC_MaxMinVec(V, Max, Min);
	double Absmx = 0, Absmn = 0;
	...
	for (size_t n = 0; n < Size; ++n)
	{
		if (C) { Print.draw_point(n, MaxMin - (V[n] + Absmn), C); }
		else { uint8_t Clr[3] = { 0, 0, 0 }; ysxCOLOR_LinearRGB(V[n], 1, 1, Clr); Print.draw_point(n, MaxMin - (V[n] + Absmn), Clr); }
	}
...
// DRAW VECTOR AS A POLYGON:
// Assumes each vector index is a polygon side, and the vector index value is the amplitude:
// REDO IT!!!
CImg<uint8_t> ysxCIMG_DRAW_VectorPolygn(std::vector<double> V, uint8_t BG, bool Vertex, uint8_t* C = nullptr)
{
	uint32_t Sides = V.size();
	double x, y;	
	std::vector<Point<int>> xy(Sides); // Pixel map
	...
	for (size_t n = 0; n < Sides; ++n)
	{
		...
		x = Cos * V[n]; y = Sin * V[n];
		if (m < Sides) { xy[m].x = round(x); xy[m].y = MaxMin - round(y); } // Add to the pixel map
		++m;
	}
...
template <class T_> CImg<uint8_t> ysxCIMG_DRAW_VectorImg(std::vector<Point3D<T_>>& V, double Rsize = 1.0)
{
	...
	SqrtVec / x == 1 ? y = x : y = ceil(SqrtVec);
	...
	for (size_t n = 0; n < Size; ++n)
	{
		...
		{
			C[0] = 255 * V[n].x / (double)pow(256, sizeof(T_));
			C[1] = 255 * V[n].y / (double)pow(256, sizeof(T_));
			C[2] = 255 * V[n].z / (double)pow(256, sizeof(T_));
		}
		Ret.draw_point(n % x, floor((float)n / x), C);
	}
	if (Rsize != 1.0) { ysxCIMG_Resize(Ret, round(x * Rsize), round(y * Rsize), 6); }
