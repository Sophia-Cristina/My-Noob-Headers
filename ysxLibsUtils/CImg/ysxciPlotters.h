...
CImg<uint8_t> ysxCIMG_DRAW_VectorPointNorm(std::vector<T_>& V)
{
	...
	{
		ndiv = (float)n / (Size - 1);
		if (std::is_floating_point<T_>::value) { ysxCIMG_CLR_LinearRGBuc(V[n], 1, 1, C); }
...
template <class T_> CImg<uint8_t> ysxCIMG_DRAW_VectorInfoLRGB(std::vector<T_>& V, double Rsize = 1.0)
{
	...
	for (size_t n = 0; n < Size; ++n)
	{
		if (std::is_floating_point<T_>::value) { ysxCIMG_CLR_LinearRGBuc(V[n], 1.0, 1.0, C); }
...
public:
	...
	void TriPeriPrint(bool AddVertex, bool Text, bool AddHeight, bool ChangeSidesColor)
	{
		...
		if (AddHeight)
		{
			...
			if (AddVertex) { uint8_t Color[] = { 127, 127, 127 }; AddVert(TriOut, (SclCrd[2].x) + Border, Offy, 3, Color); }
		}
	}
